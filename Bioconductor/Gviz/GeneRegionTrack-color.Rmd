...
- `final` and `source` are values of `as.vector(example_combined_grange$source)`
   which are mapped to the `"red"` and `"blue"` colors, respectively

```
gen <- as.character(unique(genome(example_combined_grange)))
chr <- as.character(unique(seqnames(example_combined_grange)))
grtrack_combined <- GeneRegionTrack(
    range = example_combined_grange,
    genome=gen, chromosome=chr, name="Genes",
    feature=as.vector(example_combined_grange$source), final="red", source="blue"
)
